...
namespace FileSystem
{
    internal class Program
    {
        ...
        static void WriteAsTabular(string path, string Filename, string[] data)
        {
        ...
        static void WriteAsTabular(string path, string Filename, List<Account> data)
        {
            ...
            if (!File.Exists(FilePath))
            {
                string header = string.Format("AccountId,Saldo");
                sb.AppendLine(header);
            }
            foreach (var usr in data)
            {
                sb.AppendLine(string.Format($"{usr.AccountId},{usr.Saldo}"));
            }
            File.AppendAllText(FilePath, sb.ToString()); // - string
        }
